...
public class PublisherScript : MonoBehaviour {
	...
	static void Connect()
	{
		...
		Uri uri = PSFactory.CreateURI( "publish-client-vive", "192.168.1.141", 9000 );
	...
	void Publish()
	{
		if( !is_connected )
			return;
        Position pos = new Position(transform.position.x, transform.position.y, transform.position.z);
        PositionEvent pe = new PositionEvent(MsgType.VIVE, pos);
        ...
        DirectionEvent de = new DirectionEvent(MsgType.VIVE, dir);
